...
import { useEvaluadores } from "@/hooks/dashboard/evaluadores/useEvaluadores";
...
/* ─── modal crear / editar ───────────────────────────────── */
function EvaluadorModal({ evaluador, onClose, onGuardar }) {
  const isEditing = Boolean(evaluador);
  const [form, setForm] = useState(
    isEditing
      ? { firstName: evaluador.firstName, lastName: evaluador.lastName, email: evaluador.email, phone: evaluador.phone, position: evaluador.position }
      : { ...EMPTY_FORM }
  );
  ...
  function handleChange(e) {
    ...
    const parsed = name === "phone" ? value.replace(/\D/g, "") : value;
  ...
  async function handleSubmit(e) {
    ...
    setSaving(false);
    if (res.ok) {
      showToast.success(isEditing ? "Evaluador actualizado." : "Evaluador creado.", { position: "top-right", duration: 3000 });
      onClose();
    } else {
        ...
        <div className="flex items-center justify-between px-6 py-4 border-b border-border">
          <div>
            <h2 className="text-lg font-semibold text-foreground">
              {isEditing ? "Editar Evaluador" : "Nuevo Evaluador"}
            </h2>
            {!isEditing && (
        ...
        <form onSubmit={handleSubmit} className="px-6 py-5 flex flex-col gap-4">
          ...
          <div className="grid grid-cols-2 gap-4">
            <div className="flex flex-col gap-1.5">
              ...
              <input name="phone" type="tel" inputMode="numeric" value={form.phone} onChange={handleChange}
                className="px-3 py-2 text-sm border border-border rounded-lg bg-background text-foreground focus:outline-none focus:ring-2 focus:ring-primary" />
            </div>
            <div className="flex flex-col gap-1.5">
...
export default function Evaluadores() {
  ...
  }, [evaluadores, search, estadoFilter]);
    ...
    setDialog({
      ...
      onConfirm:    async () => {
        ...
        showToast[res.ok ? "success" : "error"](
          res.ok ? "Evaluador desactivado." : (res.error ?? "No se pudo desactivar."),
  ...
  function confirmarReactivar(u) {
    setDialog({
      ...
      onConfirm:    async () => {
        ...
        showToast[res.ok ? "success" : "error"](
          res.ok ? "Evaluador reactivado." : (res.error ?? "No se pudo reactivar."),
          { position: "top-right", duration: 4000 }
        );
      },
    });
  ...
  return (
    <div className="flex flex-col h-full" onClick={() => setShowEstadoMenu(false)}>
      ...
      <div className="flex items-start justify-between px-8 py-6 bg-card border-b border-border">
        <div>
          <h1 className="text-2xl font-bold text-foreground">Evaluadores</h1>
          <p className="text-muted-fg text-sm mt-0.5">Gestiona los evaluadores y sus accesos a la plataforma</p>
        </div>
        <button
      ...
      <div className="flex-1 px-8 py-5 flex flex-col gap-4 overflow-y-auto">
        ...
        <div className="flex items-center gap-3">
          ...
          {(search || estadoFilter !== "Todos") && (
            <button onClick={() => { setSearch(""); setEstadoFilter("Todos"); resetPage(); }}
        ...
        <div className="bg-card border border-border rounded-xl overflow-hidden">
          <table className="w-full text-sm">
            <thead>
              <tr className="border-b border-border">
                <th className="text-left px-6 py-3 text-muted-fg font-medium">Evaluador</th>
              ...
              {loading ? (
                ...
                <tr>
                  <td colSpan={5} className="px-6 py-12 text-center text-muted-fg text-sm">
                    {filtered.length === 0 && evaluadores.length > 0
                ...
                paginated.map((u) => (
                  <tr key={u.id} className="border-b border-border last:border-0 hover:bg-background transition-colors">
                    <td className={`px-6 py-3 ${!u.isActive ? "opacity-50" : ""}`}>
                      <div className="flex items-center gap-3">
                        <Avatar nombre={u.nombre} />
                        <span className="font-medium text-foreground">{u.nombre}</span>
                      </div>
                    </td>
                    <td className={`px-6 py-3 text-muted-fg ${!u.isActive ? "opacity-50" : ""}`}>{u.email}</td>
                    <td className={`px-6 py-3 text-foreground ${!u.isActive ? "opacity-50" : ""}`}>{u.position || "—"}</td>
                    <td className={`px-6 py-3 ${!u.isActive ? "opacity-50" : ""}`}>
                    ...
                    <td className="px-6 py-3">
                      <div className="flex items-center gap-1.5">
                        <button
                          onClick={() => setModal({ mode: "editar", evaluador: u })}
          ...
          <div className="px-6 py-3 border-t border-border flex items-center justify-between">
            <span className="text-muted-fg text-xs">
              {loading
                ? "Cargando…"
                : `Mostrando ${paginated.length} de ${filtered.length} evaluador${filtered.length !== 1 ? "es" : ""}`}
            </span>
      ...
      {modal && (
        <EvaluadorModal
          evaluador={modal.mode === "editar" ? modal.evaluador : null}
          onClose={() => setModal(null)}
          onGuardar={(data) =>
            modal.mode === "editar"
              ? handleActualizar(modal.evaluador.id, data)
